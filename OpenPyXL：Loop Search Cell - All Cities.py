...
# *** Custom Filter *** #
custom_filter_list = ["中国", "首都", "虹桥", "自治", "自治区", "高新", "高新区", "天府", "新区", "天府新区", "郑东", "亚运村",
                      "人民", "政府", "西藏自治区人民政府", "驻", "办事处", "科", "附属", "国际", "学府", "街", "服务", "浆洗街",
                      "综合", "开放", "医疗", "医科", "医美", "医生", "医学美容", "医学", "医学院", "医学整形", "医学中心", "医药",
                      "医院", "医疗器械", "美容", "美容外科", "美容中心", "美体", "美容美发", "华医", "总医院",
                      "眼科医院", "妇产医院", "妇儿", "妇儿医院", "妇幼保健", "保健", "院", "专科门诊", "激光", "专科",
                      "口腔医院", "盆底", "康复", "诊所", "门诊", "门诊部", "综合", "结合", "特色", "健康", "皮肤", "分公司",
                      "连锁", "有限", "有限公司", "有限责任", "企业", "投资", "管理", "咨询", "普通", "合伙", "公司", "集团",
                      "股份", "科技开发", "科技", "技术", "开发", "研究", "研究院", "顾问", "宁夏", "广西", "新疆",
                      "整形", "整形外科", "整形医院", "中西医", "中医", "西医", "中心", "中医院", "中国人民解放军",
                      "大学", "复旦", "复旦大学", "交通", "交通大学", "上海交通大学", "中医药大学", "首都医科大学", "四川大学",
                      "（", "）", "(", ")", "奥克斯", "广场", "店", "长宁店", "百货店", "新世界", "京西", "院区"]
...
# *** Load Hospital Data *** #
wb1 = openpyxl.load_workbook("data/Generated - All Cities Galderma Hospital List.xlsx")
...
# wb1 = openpyxl.load_workbook("data/Generated - Guangzhou Hospital List.xlsx")
# wb1 = openpyxl.load_workbook("data/Generated - Shenzhen Hospital List.xlsx")
...
def hospital_mapping(keyword, city):
    # *** 1st SoYoung Mapping *** #
...
for Galderma_Hospital_cell in Galderma_hospital_B_columns:
    ...
    if Galderma_Hospital_cell.value is not None and Galderma_Hospital_cell.value != "-" and "VLOOKUP" not in Galderma_Hospital_cell.value:
        ...
        print("Row Index: " + str(Galderma_hospital_row_index))
        keyword = "".join(keyword_list)
        print("Keyword：" + keyword)
        city = str(sheet1['G' + str(Galderma_hospital_row_index)].value)
        if "自治州" in city:
            city = city[:len(city)-3]
        else:
            city = city[:len(city)-1]
        print("City：" + city)
        if len(keyword) > 1 and city:
            hospital_mapping(keyword, city)
...
wb1.save("data/Generated - All Cities Galderma Hospital List.xlsx")
# wb1.save("data/Generated - Shanghai Hospital List.xlsx")
# wb1.save("data/Generated - Beijing Hospital List.xlsx")
# wb1.save("data/Generated - Guangzhou Hospital List.xlsx")
# wb1.save("data/Generated - Shenzhen Hospital List.xlsx")
